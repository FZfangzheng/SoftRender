...
//获取阴影贴图
void rst::rasterizer::getShadowMap() {
    ...
    for (auto& kv : obj_map) {
        for (const auto& t : kv.second)
        {
            ...
            std::array<Eigen::Vector3f, 3> viewspace_pos = vertex_shader_shadow_map[kv.first]->use(&newtri, t);
            // 光栅化显示三角形
            rasterize_triangle_shadow(kv.first, newtri, viewspace_pos);
        }
    }
}
...
//}
// 透视投影矫正-中心插值
static Eigen::Vector3f interpolate(float alpha, float beta, float gamma, const Eigen::Vector3f& vert1, const Eigen::Vector3f& vert2, const Eigen::Vector3f& vert3, float z1, float z2, float z3, float zt)
{
    return (alpha * vert1 / z1 + beta * vert2 / z2 + gamma * vert3 / z3)* zt;
}
static Eigen::Vector2f interpolate(float alpha, float beta, float gamma, const Eigen::Vector2f& vert1, const Eigen::Vector2f& vert2, const Eigen::Vector2f& vert3, float z1, float z2, float z3, float zt)
{
    return (alpha * vert1 / z1 + beta * vert2 / z2 + gamma * vert3 / z3) * zt;
}
...
//阴影贴图
void rst::rasterizer::rasterize_triangle_shadow(int idx, const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
    auto v = t.v;
    // 三角形三个点
    Vector4f A = v[0];
    ...
    for (int x = std::max(minX_i, 0); x <= std::min(maxX_i, width - 1); ++x) {
        for (int y = std::max(minY_i, 1); y <= std::min(maxY_i, height); ++y) {
            if (insideTriangle(static_cast<float>(x) + 0.5, static_cast<float>(y) + 0.5, t.v)) {
                ...
                // 用于透视投影矫正，Z为观察空间的深度值，w为各个点在观察空间的深度
                ...
                if (zp > depth_buf[get_index(x, y)]) {
                    ...
                    // 有一些奇怪的点，通过上面的深度测试，但是下面的Z会被覆盖更小的值，问题待查
                    if (Z > shadow_buf[get_index(x, y)]) {
                        shadow_buf[get_index(x, y)] = Z;
                    }
        ...
        for (int y = std::max(minY_i, 1); y <= std::min(maxY_i, height); ++y) {
            if (insideTriangle(static_cast<float>(x) + 0.5, static_cast<float>(y) + 0.5, t.v)) {
                ...
                if (zp > depth_buf[get_index(x, y)]) {
                    ...
                    // color
                    auto interpolated_color = interpolate(alpha, beta, gamma, t.color[0], t.color[1], t.color[2], v[0].w(), v[1].w(), v[2].w(), Z);
                    // normal
                    auto interpolated_normal = interpolate(alpha, beta, gamma, t.normal[0], t.normal[1], t.normal[2], v[0].w(), v[1].w(), v[2].w(), Z).normalized();
                    // texcoords
                    auto interpolated_texcoords = interpolate(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1], t.tex_coords[2], v[0].w(), v[1].w(), v[2].w(), Z);
                    // shadingcoords
                    auto interpolated_shadingcoords = interpolate(alpha, beta, gamma, view_pos[0], view_pos[1], view_pos[2], v[0].w(), v[1].w(), v[2].w(), Z);
                    ...
                    auto pixel_color = fragment_shader_map[idx]->use(x, y, payload, shadow_buf);
                    // 设置深度
                    depth_buf[get_index(x, y)] = zp;
                    // 设置颜色
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    ...
    // 深度值设置为最小
    if ((buff & rst::Buffers::Shadow) == rst::Buffers::Shadow)
    {
        std::fill(shadow_buf.begin(), shadow_buf.end(), std::numeric_limits<float>::lowest());
    }
}
// 初始化缓冲大小
...
//}
// 设置顶点着色器
void rst::rasterizer::set_vertex_shader(int idx, VertexShader* vs)
{
    vertex_shader_map.emplace(idx, vs);
}
// 设置片元着色器
void rst::rasterizer::set_fragment_shader(int idx, FragmentShader* fs)
{
    fragment_shader_map.emplace(idx, fs);
}
// 设置顶点着色器-阴影
void rst::rasterizer::set_vertex_shadow_shader(int idx, VertexShader* vs)
{
    vertex_shader_shadow_map.emplace(idx, vs);
}
// 设置片元着色器-阴影
void rst::rasterizer::set_fragment_shadow_shader(int idx, FragmentShader* fs)
{
    fragment_shader_shadow_map.emplace(idx, fs);
}
